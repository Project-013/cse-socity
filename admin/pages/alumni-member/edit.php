        ...
        elseif (mysqli_num_rows(mysqli_query($conn, "select * from members where email='$email' and id != '".$_GET['id'] ."'"))) {
            ...
            if ($_FILES["avatar"]["name"]) {
                ...
                if (move_uploaded_file($_FILES["avatar"]["tmp_name"], $avatar)) {
                    ...
                    $sql = mysqli_query($conn, "UPDATE members SET avatar='$avatar', name='$name',email= '$email', phone='$phone', designation='$designation',address='$address', time_range='$time_range', short_details='$short_details', details='$details' WHERE id='".$_GET['id'] ."'");
                    if($sql)
                    {
            ...
            {
                $sql1 = mysqli_query($conn, "UPDATE members SET name='$name',email= '$email', phone='$phone', designation='$designation',address='$address', time_range='$time_range', short_details='$short_details', details='$details' WHERE id='".$_GET['id'] ."'");
                if($sql1)
                {
                    $_SESSION['success'] = 'Data Update Successfully';
                }
