    ...
    <div class="col-12">
        <div class="card ">
            <div class="card-header bg-info">
                <a href="index.php?page=add-service"
                    class="btn waves-effect waves-light btn-outline-light float-end">Add
                    New Service</a>

                <h3 class="m-b-0 text-white"><b>Service</b></h3>
                    ...
                    <table id="myTable" class="table table-striped border">
                        ...
                        if (mysqli_num_rows($q) > 0) {
                            ...
                            <tr>
                                <td><?= $i ?></td>
                                <td>
                                    <img src="<?= $row['logo'] ?>" style="width:80px;height:50px;" alt="">
                                </td>
                                <td>
                                    <h5>
                                        <b><?= $row['title'] ?></b>
                                ...
                                <td class="">
                                    <a class="m-0 <?= $row['status' ] == 0 ? 'text-warning': 'text-success'; ?>"
                                        href="index.php?page=service-list&status_id=<?= $row['id']; ?>">
                                        <i style="font-size:20px"
                                            class="mdi mdi-numeric-<?= $row['status' ] == 0 ? '0-box' : '1-box'; ?>"
                                            style=""></i>
                                    </a>
                                    <a class="" href="index.php?page=edit-service&id=<?= $row['id'] ?>" title="Edit"><i
                                            class="mdi mdi-lead-pencil text-info" style="font-size:20px"></i>
                                    </a>
                                    <a class="" href="index.php?page=service-list&delete_id=<?= $row['id'] ?>"
...
if (isset($_GET['status_id'])) {
    $q = mysqli_query($conn, "SELECT * FROM service WHERE id='" . $_GET['status_id'] . "'");
    ...
    if ($row) {
        ...
        $q1 = mysqli_query(
            $conn,
            "UPDATE service SET status ='$stt' WHERE id='" .$_GET['status_id'] . "'" );
        if ($q1) {
...
<?php if (isset($_GET['delete_id'])) {
    if (
        mysqli_query(
            $conn,
            "delete from service where id='" . $_GET['delete_id'] . "'"
        )
    ) {
