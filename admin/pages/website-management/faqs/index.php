    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header bg-info">
                <a href="index.php?page=add-faqs" class="btn waves-effect waves-light btn-outline-light float-end">Add
                    New FAQs</a>

                <h3 class="m-b-0 text-white"><b>FAQs</b></h3>
                            ...
                            <tr>
                                ...
                                <td class="">
                                    <a class="m-0 <?= $row['status' ] == 0 ? 'text-warning': 'text-success'; ?>"
                                        href="index.php?page=faqs-list&status_id=<?= $row['id']; ?>">
                                        <i style="font-size:20px"
                                            class="mdi mdi-numeric-<?= $row['status' ] == 0 ? '0-box' : '1-box'; ?>"
                                            style=""></i>
                                    </a>
                                    <a class="" href="index.php?page=edit-faqs&id=<?= $row['id'] ?>" title="Edit"><i
                                            class="mdi mdi-lead-pencil text-info" style="font-size:20px"></i>
                                    </a>
                                    <a class="" href="index.php?page=faqs-list&delete_id=<?= $row['id'] ?>"
...
if (isset($_GET['status_id'])) {
    $q = mysqli_query($conn, "SELECT * FROM faqs WHERE id='" . $_GET['status_id'] . "'");
    ...
    if ($row) {
        ...
        $q1 = mysqli_query(
            $conn,
            "UPDATE faqs SET status ='$stt' WHERE id='" .$_GET['status_id'] . "'" );
        if ($q1) {
...
<?php if (isset($_GET['delete_id'])) {
    if (
        mysqli_query(
            $conn,
            "delete from faqs where id='" . $_GET['delete_id'] . "'"
        )
    ) {
